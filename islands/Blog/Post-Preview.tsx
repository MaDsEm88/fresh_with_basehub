...
export function PostPreview({
  ...
  const handleClickCopySlug = () => {
    const slugToCopy = `https://localhost:8001/${slug}`;
  ...
  return (
              ...
              {copiedSlug ? (
                ...
                <svg width="15" height="15" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                  ...
                  <g fill="none" className="nc-icon-wrapper">
                    <path className="fill-white" d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41L9 16.17z" 
                    ></path>
                  </g>
                </svg>
              ) : (
                ...
                <svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" fill="none" viewBox="0 0 32 32">
                  <path stroke="#efefef" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14.9 19.142l-.708-.707c-2.121-2.121-2.121-4.95 0-7.071l2.829-2.828s0 0 0 0c2.121-2.122 4.95-2.122 7.07 0 2.122 2.12 2.122 4.95 0 7.07"/>
                  <path stroke="#efefef" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.728 13.485l.707.707c2.121 2.122 2.121 4.95 0 7.072l-2.828 2.828s0 0 0 0c-2.122 2.121-4.95 2.121-7.071 0-2.122-2.121-2.122-4.95 0-7.071"/>
                  <title>url_copy</title>
                </svg>
              )}
              {/* Render "Copied" or "Copy link" based on copied state */}
              <span className="hidden text-white/80 md:block">{copiedSlug ? 'Copied' : 'Copy link'}</span>
            </button>
        </div>
    ...
    <Toaster  position="bottom-right" expand={false} /> {/* Include the Toaster component */}
  </section>
  );
}